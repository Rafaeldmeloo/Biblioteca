...
void ClienteVip::AdicionaEmprestimo(Emprestimo emp){
        std::cout << std::endl;
        std::cout << "----Empréstimo adicionado com sucesso----" << std::endl;
...
void ClienteVip::MostrarLista(){
    setlocale(LC_ALL, "Portuguese");
        ...
        std::cout << std::endl;
        std::cout << "Cidade: " << emprestimo.getCidade() << std::endl;
...
int ClienteVip::ExcluirEmprestimo(std::string procuraL, std::string procuraN, std::string cidade){
    setlocale(LC_ALL, "Portuguese");

    if(emprestimo.getNomes().find(procuraN) != std::string::npos && cidade == emprestimo.getCidade()){
        if(emprestimo.getLivros().find(procuraL) != std::string::npos ){
            ...
            if(op == 2)
                ...
                emprestimo.setCidade(emprestimo2.getCidade());
                emprestimo2.setLivros("");
                emprestimo2.setNomes("");
                emprestimo2.setCidade("");
...
bool ClienteVip::AlterarEmprestimo(std::string procuraL, std::string procuraN, std::string cidade){
    setlocale(LC_ALL, "Portuguese");
    int alterar = -1;
...
int ClienteVip::opcaoEditar(){
  ...
  while(1){
    std::cout << std::endl;
    std::cout << "-------Edição de empréstimos-------" << std::endl;
